...
import torch
from model import FeatureExtractor, UserClassifier, BehaviorClassifier, BANNModel
...
def testModel(testDataset, model, UserNum, batchSize, legalUser):
    ...
    for data in testSet1_loader:
        ...
        class_outputs, _ = model(samples, 0)
...
class CustomDataset(Dataset):
    ...
    def from_file(cls, path):
        ...
        return cls(data['samples'], data['labels'])
...
if __name__ == '__main__':
    ...
    dataType1 = CustomDataset.from_file('Dataset/chair'+'.pth')
    dataType2 = CustomDataset.from_file('Dataset/head'+'.pth')
    dataType3 = CustomDataset.from_file('Dataset/body'+'.pth')
    dataType4 = CustomDataset.from_file('Dataset/walk'+'.pth')
    ...
    for legalUser in legalUser_list:
        ...
        user_classifier.load_state_dict(torch.load('user_classifier.pth'))

        behavior_classifier = BehaviorClassifier()
        behavior_classifier.load_state_dict(torch.load('behavior_classifier.pth'))

        my_net.feature_extractor = feature_extractor
        my_net.gru_classifier = user_classifier
        my_net.transformer_classifier = behavior_classifier
        ...
        for epoch in range(n_epoch):
            ...
            len_dataloader = len(dataType1_loader)
        ...
        with torch.no_grad():
            ...
            BAC_chairList.append(BAC1)
            print("chair, FAR1, FRR1, TPR1, TNR1, BAC1:", FAR1, FRR1, TPR1, TNR1, BAC1)

            FAR2, FRR2, TPR2, TNR2, BAC2 = testModel(testType2, my_net, len(UserList), batchSize, legalUser)
            BAC_headList.append(BAC2)
            print("head, FAR2, FRR2, TPR2, TNR2, BAC2:" , FAR2, FRR2, TPR2, TNR2, BAC2)

            FAR3, FRR3, TPR3, TNR3, BAC3 = testModel(testType3, my_net, len(UserList), batchSize, legalUser)
            BAC_bodyList.append(BAC3)
            print("body, FAR3, FRR3, TPR3, TNR3, BAC3:",  FAR3, FRR3, TPR3, TNR3, BAC3)

            FAR4, FRR4, TPR4, TNR4, BAC4 = testModel(testType4, my_net, len(UserList), batchSize, legalUser)
            BAC_walkList.append(BAC4)
            print("walk, FAR4, FRR4, TPR4, TNR4, BAC4:",  FAR4, FRR4, TPR4, TNR4, BAC4)
    ...
    print("BAC_chair, BAC_head, BAC_body, BAC_walk:", np.mean(BAC_chairList), np.mean(BAC_headList), np.mean(BAC_bodyList),
          np.mean(BAC_walkList))
